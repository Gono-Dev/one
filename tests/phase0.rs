...
async fn admin_settings_page_is_read_only_config_view() {
    ...
    assert!(body.contains("TLS cert file"));
    assert!(body.contains("auth_realm"));
    assert!(body.contains("notify_push_pre_auth_ttl_secs"));
    assert!(body.contains("admin_users"));
    assert!(body.contains("Upload minimum free bytes"));
